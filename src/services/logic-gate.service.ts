  ...
  test(
    ...
    const conditions: [Condition, true | Condition][] =
      logicGate.conditions.map((condition) => [
        condition,
        ConditionService.testCondition(condition, context, debug),
      ]);
    ...
    switch (logicGate.type) {
      case BooleanLogicTypeEnum.AND: {
        return this._testAnd(logicGate as AndConditionDto, conditions, debug);
      }
      case BooleanLogicTypeEnum.OR: {
        return this._testOr(logicGate as OrConditionDto, conditions, debug);
      }
      case BooleanLogicTypeEnum.NAND: {
        return this._testNAnd(logicGate as NandConditionDto, conditions, debug);
      }
      case BooleanLogicTypeEnum.NOR: {
        return this._testNor(logicGate as NorConditionDto, conditions, debug);
      }
      case BooleanLogicTypeEnum.XOR: {
        return this._testXOr(logicGate as XorConditionDto, conditions, debug);
      }
      default:
        if (logicGate.debug || debug) {
          console.log(
